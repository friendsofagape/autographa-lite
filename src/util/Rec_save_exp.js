...
module.exports = {
    recSave: function (book, file, chapter) {
        ...
        var filePath = `${appPath}/recordings/${book.bookName}/${chapter}/${chapter} ${Date.now()}.webm`;
        fs.exists(`${appPath}/recordings/${book.bookName}/${chapter}`, function(exists) {
            if (exists) {
                ...
                fs.exists(`${appPath}/recordings`, function(exists) {if (exists) {console.log("exists")}
                else fs.mkdirSync(`${appPath}/recordings`)})

                fs.exists(`${appPath}/recordings/${book.bookName}`, function(exists) {if (exists) {console.log("exists")}
                else fs.mkdirSync(`${appPath}/recordings/${book.bookName}`)})
                
                fs.exists(`${appPath}/recordings/${book.bookName}/${chapter}`, function(exists) {if (exists) {console.log("exists")}
