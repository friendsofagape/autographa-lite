							...
							for( let i=0; i<obj.verses.length; i++){
								...
								if (count < obj.verses.length && obj.verses[count].joint_verse) {
									...
									if (verseNumber) {
										...
										htmlContent += `<div><p class="prespace"><span class="verseSpan">${verseNumber}</span>${verses}</p></div>`
										verseNumber = undefined;
										verses = undefined;
									} else {
										// Push verse number and content.
										htmlContent += `<div><p class="prespace"><span class="verseSpan">${obj.verses[i].verse_number} </span>${obj.verses[i].verse}</p></div>`
									}
								}	
							}
